...
public class ICTNewsParser extends BaseParser {
	...
	private void collectICTNewsArticles(Source source, List<News> congngheNews, String type, String parentCateName,
			String fromWebsite) {
		A a = new A();
		...
		p.setExcludedTexts(
				"CNTT,Bảo mật,Nước mạnh CNTT,Phần mềm,Viễn thông,Số hóa truyền hình,Xã hội @,Internet,Thủ thuật,Di động,Khởi nghiệp,Ô tô – Xe máy,Khoa học,Điện máy,Khoa học,Máy tính");
		List<Element> articles = source.getAllElementsByClass("news-list");
		for (Element e : articles) {
			if (e.getAllElements("li") != null) {
				for (Element li : e.getAllElements("li")) {
					News n = new News();
					n.setFromWebSite(fromWebsite);
					n.setType(type);
					n.setParentCateName(parentCateName);
					parseElementToNews(li, n, a, t, image, p);
